...
const TRUST_ITEMS = [
  { icon: '🪪', title: 'Real identity required',       body: 'Every user registers with their real name, email, and phone number.' },
  { icon: '🔞', title: '18+ age verification',          body: 'Date of birth is verified on signup. Minors cannot create accounts.' },
  { icon: '💬', title: 'In-app messaging only',         body: 'All coordination happens inside the app. No personal contact info is ever exchanged.' },
  { icon: '⏱️', title: '2-hour response window',        body: 'Seekers must respond within 2 hours of claiming or the ticket is automatically re-listed.' },
  { icon: '✅', title: 'Transfer confirmation required', body: 'Listers must confirm the ticket was sent before match day or the meet is cancelled.' },
];
  ...
  return (
    <main id="main-content">
      ...
      <section className="hiw-roles container" aria-label="Lister or seeker">
        ...
        <div className="hiw-roles-grid">
          <div className="hiw-role-card hiw-role-lister" role="region" aria-label="Lister description">
            <div className="hiw-role-icon" aria-hidden="true">🎟️</div>
            <h3>Lister</h3>
          ...
          <div className="hiw-trust-grid">
            {TRUST_ITEMS.map(item => (
              <div key={item.title} className="hiw-trust-card">
                <span className="hiw-trust-icon" aria-hidden="true">{item.icon}</span>
                <div>
      ...
      <section className="hiw-cta" aria-label="Call to action">
        <div className="container">
          <h2 className="hiw-cta-title">Ready to find your bro?</h2>
